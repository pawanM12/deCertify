...
function OrganizationDashboard() {
  const { user, logout } = useAuth(); // Removed unused updateBlockchainStatus
  const { contract, address, isConnected, loading: web3Loading, error: web3Error } = useWeb3(); // Removed unused connectWallet, renamed loading to web3Loading
  ...
  const [orgIssuanceFee, setOrgIssuanceFee] = useState(0n);
  const [fetchingOrgFees, setFetchingOrgFees] = useState(true);
  ...
  }, [user]); // Changed dependency from user?.token to user
  ...
  const handleAcceptRequest = async (requestId, studentWalletAddress) => {
    ...
    try {
      ...
      const { pdfIpfsHash } = uploadResponse.data;
